...
const ApprovedPremium = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('https://match-finder-server.vercel.app/biodatas/premium-requests')
      .then(res => setRequests(res.data.requests || []))
      .catch(() => setError('Failed to load premium requests'))
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: 'The premium request has been approved.',
        timer: 1500,
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to approve premium request',
      });
    }
  };

  if (loading) return <div className="text-center py-10 text-gray-500">Loading premium requests...</div>;
  if (error) return <div className="text-center py-10 text-red-600">{error}</div>;

  if (requests.length === 0) {
    return <div className="text-center py-10 text-gray-500">No premium requests found.</div>;
  }

  return (
    <div className="max-w-4xl mx-auto p-8 bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-2xl mt-8 mb-12">
      <h2 className="text-3xl font-bold mb-8 text-blue-900">Premium Approval Requests</h2>
      <div className="overflow-x-auto">
        <table className="min-w-full border border-blue-100 divide-y divide-blue-100 bg-white rounded-xl shadow">
          ...
          <tbody className="divide-y divide-blue-100">
            {requests.map(({ biodataId, name, email }) => (
              <tr key={biodataId} className="hover:bg-blue-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <button
                    onClick={() => handleApprove(biodataId)}
